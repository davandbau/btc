#!/usr/bin/env python3
"""
Polymarket 5-Minute SOL Reasoning Trader — data + execution tool.
...
def get_market_brief():
    ...
    klines = fetch_json("https://api.binance.com/api/v3/klines?symbol=SOLUSDT&interval=1m&limit=15")
    if klines:
        candles = []
        for k in klines:
    ...
    prev_results = []
    for i in range(1, 4):
        prev_slug = f"sol-updown-5m-{current_window - (i * 300)}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--delta", type=float, help="SOL delta from strike at entry")
