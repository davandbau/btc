#!/usr/bin/env python3
"""
Polymarket 5-Minute ETH Reasoning Trader — data + execution tool.
...
def get_market_brief():
    ...
    if book:
        ...
        brief["orderbook"] = {
            "bids_eth": round(bids, 3),
            "asks_eth": round(asks, 3),
    ...
    klines = fetch_json("https://api.binance.com/api/v3/klines?symbol=ETHUSDT&interval=1m&limit=15")
    if klines:
        candles = []
        for k in klines:
    ...
    prev_results = []
    for i in range(1, 4):
        prev_slug = f"eth-updown-5m-{current_window - (i * 300)}"
...
def record_trade(side, entry_price, reasoning, position_size=POSITION_SIZE,
    ...
    eth_price = cl[0]["price"] if cl else 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--delta", type=float, help="ETH delta from strike at entry")
