#!/usr/bin/env python3
"""
Polymarket 5-Minute XRP Reasoning Trader — data + execution tool.
...
def get_market_brief():
    ...
    brief = {
        ...
        "window_end_utc": datetime.fromtimestamp(current_window + 300, tz=timezone.utc).strftime("%H:%M:%S"),
    ...
    klines = fetch_json("https://api.binance.com/api/v3/klines?symbol=XRPUSDT&interval=1m&limit=15")
    if klines:
        candles = []
        for k in klines:
    ...
    prev_results = []
    for i in range(1, 4):
        prev_slug = f"xrp-updown-5m-{current_window - (i * 300)}"
...
def record_trade(side, entry_price, reasoning, position_size=POSITION_SIZE,
    ...
    trade = {
        ...
        "time_remaining": round((current_window + 300) - now, 1),
        "reasoning": reasoning,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--delta", type=float, help="XRP delta from strike at entry")
