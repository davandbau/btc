#!/usr/bin/env python3
"""
Polymarket 5-Min XRP Reasoning Loop — Single Tranche, Momentum-First.

Watches 5-min XRP Up/Down windows, spawns OpenClaw sub-agent for trade decisions.
...
CHAINLINK_FEED_ID = "0x0003c16c6aed42294f5cb4741f6e59ba2d728f0eae2eb9e6d3f555808c59fc45"
...
def build_brief(cached_strike=None):
    ...
    # Binance order book
    book = fetch_json("https://api.binance.com/api/v3/depth?symbol=XRPUSDT&limit=20")
    if book:
    ...
    trades = fetch_json("https://api.binance.com/api/v3/aggTrades?symbol=XRPUSDT&limit=500")
    if trades:
    ...
    for tf, key, count in [("1m", "candles_1m", 30), ("15m", "candles_15m", 8), ("1h", "candles_1h", 6)]:
        klines = fetch_json(f"https://api.binance.com/api/v3/klines?symbol=XRPUSDT&interval={tf}&limit={count}")
        if klines:
            candles = []
            closes = []
            highs = []
    ...
    slug = f"xrp-updown-5m-{current_window}"
    ...
    htf_candles = fetch_json("https://api.binance.com/api/v3/klines?symbol=XRPUSDT&interval=1h&limit=12")
    ...
    funding = fetch_json("https://fapi.binance.com/fapi/v1/premiumIndex?symbol=XRPUSDT")
    if funding:
        brief["funding_rate"] = round(float(funding.get("lastFundingRate", 0)) * 100, 4)

    # Binance futures open interest
    oi = fetch_json("https://fapi.binance.com/fapi/v1/openInterest?symbol=XRPUSDT")
    if oi:
        brief["open_interest_btc"] = round(float(oi.get("openInterest", 0)), 2)

    # Futures price (for basis/spread)
    futures_price = fetch_json("https://fapi.binance.com/fapi/v1/ticker/price?symbol=XRPUSDT")
    ...
    taker = fetch_json("https://fapi.binance.com/fapi/v1/ticker/24hr?symbol=XRPUSDT")
    if taker:
    ...
    # Long/Short ratio
    ls = fetch_json("https://fapi.binance.com/futures/data/globalLongShortAccountRatio?symbol=XRPUSDT&period=5m&limit=1")
    if ls and len(ls) > 0:
    ...
    coinbase = fetch_json("https://api.coinbase.com/v2/prices/XRP-USD/spot")
    ...
    for tf, period, weight in [("1h", 48, 1), ("4h", 42, 2), ("1d", 30, 1)]:
        kl = fetch_json(f"https://api.binance.com/api/v3/klines?symbol=XRPUSDT&interval={tf}&limit={period}")
        if kl and len(kl) >= 21:
    ...
    # Funding rate
    fr = fetch_json("https://fapi.binance.com/fapi/v1/fundingRate?symbol=XRPUSDT&limit=3")
    if fr:
...
def get_observation_snapshot():
    ...
    trades = fetch_json("https://api.binance.com/api/v3/aggTrades?symbol=XRPUSDT&limit=200")
    if trades:
        buy_vol = sum(float(t["q"]) for t in trades if not t["m"])
        sell_vol = sum(float(t["q"]) for t in trades if t["m"])
        total = buy_vol + sell_vol
    ...
    for tf, label in [("15m", "15m"), ("1h", "1h")]:
        klines = fetch_json(f"https://api.binance.com/api/v3/klines?symbol=XRPUSDT&interval={tf}&limit=6")
        if klines:
    ...
    # RSI from 1m candles
    klines_1m = fetch_json("https://api.binance.com/api/v3/klines?symbol=XRPUSDT&interval=1m&limit=15")
    ...
    # Funding rate
    funding = fetch_json("https://fapi.binance.com/fapi/v1/fundingRate?symbol=XRPUSDT&limit=1")
    if funding:
...
def run_loop(dry_run=False, live=False):
    ...
    print(f"{'='*65}")
    print(f"◈ Polymarket 5-Min XRP Reasoning Loop — Monitoring Window")
    ...
    # 2. Binance spot API
    try:
        ticker = fetch_json("https://api.binance.com/api/v3/ticker/price?symbol=XRPUSDT")
        if ticker and "price" in ticker:
            checks.append(("Binance spot", f"✅ XRP=${float(ticker['price']):,.0f}"))
        else:
    ...
    try:
        fut = fetch_json("https://fapi.binance.com/fapi/v1/ticker/price?symbol=XRPUSDT")
        if fut and "price" in fut:
            checks.append(("Binance futures", f"✅ XRP=${float(fut['price']):,.0f}"))
        else:
    ...
    PHASE_OFFSET = 17
    print(f"   Phase offset: {PHASE_OFFSET}s (XRP)")
                        ...
                        if cl_data and "data" in cl_data:
                            ...
                            if best_node and best_gap < 30:  # sanity: within 30s
                                ...
                                if True:
                                    ...
                                    _pm_strike_str = ""
                                    try:
                                        _pm_url = f"https://clob.polymarket.com/markets/xrp-updown-5m-{int(current_window)}"
